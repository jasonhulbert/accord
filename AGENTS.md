...
- `adapters/claude/` — self-contained Claude skills (`expedition/`, `expedition-missive/`), each with vendored creed, templates, and spec essentials. The repo is the source of truth; the skill folders are build targets produced by `tools/build-skill`.
- `adapters/codex/` — self-contained Codex skills (`expedition/`, `expedition-missive/`) with Codex metadata and vendored creed, templates, and spec essentials. The repo is the source of truth; shared files are build targets produced by `tools/build-skill`.
