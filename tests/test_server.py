...
class ServerTests(unittest.TestCase):
    ...
    def test_server_lists_records_and_explicitly_refreshes_a_task_view_from_disk(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn('<nav class="view-nav"><a href="/">Refresh</a></nav>', index)
            ...
            self.assertIn("top: 0", index)
            self.assertIn("background: var(--paper)", index)
            self.assertIn('<div class="page-context">', index)
            self.assertIn("font-size: clamp(2rem, 5vw, 3.6rem)", index)
            ...
            self.assertIn('<div class="page-context">', page)
            self.assertIn("font-size: clamp(2rem, 5vw, 3.6rem)", page)
