...
- **Missives** are the patron speaking first mid-journey — a request for an account, news, or changed terms. The `expedition-missive` skill is the explicit channel; when the patron speaks first without invoking it, the ceremony applies all the same: receive the missive per `spec/missive.md`.
